...
unsigned long long llrand() {
    ...
    /* Запись long long занимает 64 бита, а стандартный оператор rand() позволяет
    генерировать значение лишь до 32767 (15 бит).
    Для покрытия всех битовых значений, используется побитовый сдвиг(15 раз за итерацию).
    По завершению цикла, биты полученного значения обрезаются, в соответствии с
...
void * getSumOfAllChilds_Pthread ( void * args ) {
    ...
    if (arg->tree != NULL) {
        ...
        if (arg->tree->left != NULL) {
            ...
            leftCreateStatus = pthread_create(&leftThread, NULL, getSumOfAllChilds_Pthread,
                                              (void *) &leftArg);
        ...
        if (arg->tree->right != NULL) {
            ...
            rightCreateStatus = pthread_create(&rightThread, NULL, getSumOfAllChilds_Pthread,
                                               (void *) &rightArg);
